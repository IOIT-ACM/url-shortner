...
import { AlertCircle, Link2Off, ChevronUp, ChevronDown, ArrowUpDown, Copy } from "lucide-react"
	...
	const columns = useMemo(
		() => [
			...
			columnHelper.accessor("original_url", {
				...
				cell: (info) => (
					<div className="flex items-center gap-2 group max-w-[200px] md:max-w-[400px]">
						<div onClick={() => handleCopy(info.getValue())} className="text-sm text-black/60 truncate font-medium cursor-pointer hover:underline">
			...
			columnHelper.accessor("instagram_mode", {
				...
				size: 100,
				cell: (info) => (
					<span className={`text-[9px] font-black uppercase tracking-tighter px-2 py-1 rounded border ${info.getValue() ? "bg-accent/5 border-accent/20 text-accent" : "bg-black/5 border-black/10 text-black/40"
						}`}>
			...
			{!isLoading && !isError && (
				<div className="mb-10 flex justify-center">
					<div className="inline-flex flex-col items-center bg-black/[0.03] px-10 py-6 rounded-3xl border border-black/5">
						<span className="text-[10px] font-black uppercase tracking-[0.2em] text-black/30 mb-2">Total Links</span>
						<span className="text-4xl font-black text-black tabular-nums">{data?.total ?? 0}</span>
					</div>
				</div>
			)}

			<div className="overflow-hidden border border-black/10 rounded-2xl bg-white shadow-sm mb-10">
				<div className="overflow-x-auto">
					<table className="w-full table-fixed border-collapse">
						<thead>
							{table.getHeaderGroups().map((headerGroup) => (
								<tr key={headerGroup.id} className="border-b border-black/10 bg-black/[0.02]">
										...
										>
											<div className="flex items-center gap-2">
												...
												{{
													...
													desc: <ChevronDown size={14} className="text-accent" />,
						...
						<tbody className="divide-y divide-black/[0.05]">
							...
							{!isLoading && isError && (
								<tr>
									<td colSpan={columns.length} className="py-20 text-center">
										<AlertCircle className="mx-auto mb-3 text-error" size={24} />
										<p className="text-sm font-bold text-black/60">Failed to load links</p>
										<button
											onClick={() => refetch()}
							...
							{!isLoading && !isError && data?.links.length === 0 && (
								<tr>
									<td colSpan={columns.length} className="py-20 text-center">
										<Link2Off className="mx-auto mb-3 text-black/10" size={32} />
										<p className="text-sm font-medium text-black/30">No links created yet</p>
									</td>
								</tr>
							)}
						</tbody>
